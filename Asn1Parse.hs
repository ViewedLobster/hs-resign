...
itmIntern :: ByteString -> [(Word8, ByteString)]
itmIntern bs | BS.null bs = []
...
derTagExtended = liftM (L.foldl' accum 0) derTagExtendedBytes
...
lenOctetsToInt :: ByteString -> Int
lenOctetsToInt = BS.foldl' accum 0
...
data DerValue = DerValue DerId Int ByteString
    deriving (Eq, Show)
...
fromIntegerDerOctetsL :: [Word8] -> Integer
fromIntegerDerOctetsL (b:bs) =
                       let negative = 0x80 .&. b == 0
                           b' = (complement 0x80) .&. b
                           abspos = L.foldl' accum 0 (b':bs)
                           absneg = L.foldl' accum 0 (L.map complement (b':bs))
                       in if negative then -absneg
                                      else abspos
    where accum acc b = (acc `shiftL` 8) .|. (fromIntegral b)

fromIntegerDerOctets = fromIntegerDerOctetsL . unpack
...
derOidValue = do
    b <- byte
    let firstTwo = L.map fromIntegral [(0xC0 .&. b) `shiftR` 6, (0x3F .&. b)] in do
        rest <- derOidRest
...
derASN1Oid = do
    ...
    case runParse derOidValue octs of -- Run inner parser of der content octets
        [(v, octs)] | BS.length octs == 0 -> return (ASN1Oid v) -- Input of parse must be depleted
        _                                 -> empt
